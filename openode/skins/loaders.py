...
from openode.skins import utils

from coffin import template
template.add_to_builtins('openode.templatetags.extra_filters_jinja')

#module for skinning openode
#via OPENODE_DEFAULT_SKIN configureation variable (not django setting)
...
OPENODE_SKIN_COLLECTION_DIR = os.path.dirname(__file__)
...
class SkinEnvironment(CoffinEnvironment):
    ...
    that loads templates from openode skins
    """
...
def get_skin(request=None):
    ...
    return SKINS[openode_settings.OPENODE_DEFAULT_SKIN]
...
def render_into_skin(template, data, request, mimetype='text/html', status_code=None):
