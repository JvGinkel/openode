...
def use_password_login():
    ...
    """
    if openode_settings.SIGNIN_WORDPRESS_SITE_ENABLED:
        return True
    if openode_settings.USE_RECAPTCHA:
        if openode_settings.RECAPTCHA_KEY and openode_settings.RECAPTCHA_SECRET:
            return True
        else:
...
def filter_enabled_providers(data):
    ...
    for provider_key, provider_settings in data.items():
        ...
        is_enabled = getattr(openode_settings, 'SIGNIN_' + name.upper() + '_ENABLED')
        if is_enabled == False:
...
class LoginMethod(object):
    ...
    as plugins for the openode's version of django_authopenid
    """
...
def add_custom_provider(func):
    ...
    def wrapper():
        providers = func()
        login_module_path = getattr(settings, 'OPENODE_CUSTOM_AUTH_MODULE', None)
...
def get_oauth_parameters(provider_name):
    ...
    if provider_name == 'twitter':
        consumer_key = openode_settings.TWITTER_KEY
        consumer_secret = openode_settings.TWITTER_SECRET
    elif provider_name == 'linkedin':
        consumer_key = openode_settings.LINKEDIN_KEY
        consumer_secret = openode_settings.LINKEDIN_SECRET
    elif provider_name == 'identi.ca':
        consumer_key = openode_settings.IDENTICA_KEY
        consumer_secret = openode_settings.IDENTICA_SECRET
    else:
...
class OAuthConnection(object):
    ...
    def start(self, callback_url=None):
        ...
        if callback_url:
            callback_url = '%s%s' % (openode_settings.APP_URL, callback_url)
