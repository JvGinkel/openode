...
class RecurseTreeExtension(Extension):
    tags = set(['recursetree'])

    def parse(self, parser):
        lineno = parser.stream.next().lineno

        # now we parse a single expression that is used as queryset_var.
        args = [parser.parse_expression()]

        # now we parse the body of the cache block up to `endrecursetree` and
        # drop the needle (which would always be `endrecursetree` in that case)
        body = parser.parse_statements(['name:endrecursetree'], drop_needle=True)

        # now return a `CallBlock` node that calls our _recursetree
        # helper method on this extension.
        return nodes.CallBlock(self.call_method('render', args),
                               [], [], body).set_lineno(lineno)

    def _render_node(self, caller, node):
        bits = []
        context.push()
    ...
    def render(self, queryset_var, caller):
        roots = cache_tree_children(queryset_var)
        bits = [self._render_node(caller, node) for node in roots]
        return ''.join(bits)
