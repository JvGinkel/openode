//var $, scriptUrl, openodeSkin
/**
 * attention - this function needs to be retired
 * as it cannot accurately give url to the media file
 */
var mediaUrl = function(resource){
    return openode['settings']['static_url'] + 'default' + '/' + resource;
};
...
var notify = function() {
    ...
    return {
        ...
        close: function(doPostback) {
            if (doPostback) {
               $.post(
                   openode['urls']['mark_read_message'],
                   { formdata: "required" }
               );
            }
            $(".notify").fadeOut("fast");
...
CommentConvertLink.prototype.createDom = function(){
    ...
    element.attr('action', openode['urls']['convertComment']);
...
FileUploadDialog.prototype.createDom = function() {
    ...
    var types = openode['settings']['allowedUploadFileTypes'];
    types = types.join(', ');
 ...
 * and become added to some organization (followers, organization members, etc.)
 * or toggle some state on/off
 * The button has four states on-prompt, off-prompt, on-state and off-state
 * on-prompt is activated on mouseover, when user is not part of organization
 * off-prompt - on mouseover, when user is part of organization
 * on-state - when user is part of organization and mouse is not over the button
 * off-state - same as above, but when user is not part of the organization
 */
...
SelectBoxItem.prototype.getData = function () {
    ...
    //with the user-organizations branch
    return {
        id: this._id,
        title: this._name,
...
// var OrganizationDropdown = function(organizations){
//     WrappedElement.call(this);
//     this._organization_list = organizations;
//     this._input_box = new TippedInput();
//     this._input_box.setInstruction('organization name');
//     this._input_box.createDom();
//     this._input_box_element = this._input_box.getElement();
//     this._input_box_element.attr('class', 'organization-name');
//     this._input_box_element.hide();
//     this._add_link = this.makeElement('a');
//     this._add_link.attr('href', '#');
//     this._add_link.attr('class', 'organization-name');
//     this._add_link.text('add new organization');
// };
// inherits(OrganizationDropdown, WrappedElement);

// OrganizationDropdown.prototype.createDom =  function(){
//   this._element = this.makeElement('ul');
//   this._element.attr('class', 'dropdown-menu');
//   this._element.attr('id', 'organizations-dropdown');
//   this._element.attr('role', 'menu');
//   this._element.attr('aria-labelledby', 'navOrganizations');

//   for (i=0; i<this._organization_list.length; i++){
//     li_element = this.makeElement('li');
//     a_element = this.makeElement('a');
//     a_element.text(this._organization_list[i].name);
//     a_element.attr('href', this._organization_list[i].link);
//     a_element.attr('class', 'organization-name');
//     li_element.append(a_element);
//     this._element.append(li_element);
//   }
// };

// OrganizationDropdown.prototype.decorate = function(element){
//   this._element = element;
//   this._element.attr('class', 'dropdown-menu');
//   this._element.attr('id', 'organizations-dropdown');
//   this._element.attr('role', 'menu');
...
// OrganizationDropdown.prototype.insertOrganization = function(organization_name, url){
//     var new_organization_li = this.makeElement('li');
//     new_organization_a = this.makeElement('a');
//     new_organization_a.attr('href', url);
//     new_organization_a.attr('class', 'organization-name');
//     new_organization_a.text(organization_name);
//     new_organization_li.append(new_organization_a);
...
//         var cleanedNewName = organization_name.toLowerCase();

//         if (listedName < cleanedNewName) {
//             if (i == links_array.length - 1){
//                 new_organization_li.insertAfter(this._element.find('li')[i-1]);
//                 break;
//             } else {
//                 continue;
//             }
//         } else if (listedName == cleanedNewName) {
//             var message = interpolate(gettext(
//                     'Organization %(name)s already exists. Organization names are case-insensitive.'
...
// OrganizationDropdown.prototype._add_organization_handler = function() {
//   var organization_name = this._input_box_element.val();
//   self = this;
...
//   $.ajax({
//     type: 'POST',
//     url: openode['urls']['add_organization'],
//     data: {organization: organization_name},
//     success: function(data){
//        if (data.success){
//          self.insertOrganization(data.organization_name, data.url);
...
// OrganizationDropdown.prototype.enableAddOrganizations = function(){
//     var self = this;
...
//         self._add_organization_handler();
...
Tag.prototype.createDom = function(){
    ...
    if (this.isLinkable()){
        var url = openode['urls']['questions'];
        var flag = false;
        var author = '';
        if (this._url_params){
