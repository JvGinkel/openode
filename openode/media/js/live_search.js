...
var liveSearch = function(query_string) {
    ...
    var search_url = openode['urls']['questions'];
    var x_button = $('input[name=reset_query]');
    var tag_warning_box = new TagWarningBox();

    //the tag search input is optional in openode
    ...
    // Wire query box
    var node_eval_handle;
    query.keyup(function(e){
        refresh_x_button();
        if (running === false){
            clearTimeout(node_eval_handle);
            node_eval_handle = setTimeout(eval_query, 400);
        }
    });
