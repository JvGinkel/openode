 ...
 * are from openode live settings and openode.const
 */
...
var validateTagLength = function(value){
    ...
    $.each(tags, function(index, value){
        if (value.length > openode['settings']['maxTagLength']){
            are_tags_ok = false;
        }
    });
    return are_tags_ok;
...
var validateTagCount = function(value){
    var tags = getUniqueWords(value);
    return (tags.length <= openode['settings']['maxTagsPerPost']);
};
...
var CPValidator = function(){
    return {
        getQuestionFormRules : function(){
            return {
                tags: {
                    required: openode['settings']['tagsAreRequired'],
                    maxlength: 105,
                    limit_tag_count: true,
                    limit_tag_length: true
                },
                text: {
                    minlength: openode['settings']['minQuestionBodyLength']
                },
                title: {
                    minlength: openode['settings']['minTitleLength']
                }
            };
        },
        getQuestionFormMessages: function(){
            return {
                tags: {
                    ...
                    maxlength: openode['messages']['tagLimits'],
                    limit_tag_count: openode['messages']['maxTagsPerPost'],
                    limit_tag_length: openode['messages']['maxTagLength']
                },
                text: {
                    ...
                    minlength: interpolate(
                                    ngettext(
                                        ...
                                        openode['settings']['minQuestionBodyLength']
                                    ),
                                    [openode['settings']['minQuestionBodyLength']]
                                )
                },
                title: {
                    ...
                    minlength: interpolate(
                                    ngettext(
                                        ...
                                        openode['settings']['minTitleLength']
                                    ),
                                    [openode['settings']['minTitleLength']]
                                )
                }
        ...
        getAnswerFormRules : function(){
            return {
                text: {
                    minlength: openode['settings']['minAnswerBodyLength']
                }
            };
        },
                ...
                text: {
                    ...
                    minlength: interpolate(
                                    ngettext(
                                        ...
                                        openode['settings']['minAnswerBodyLength']
                                    ),
                                    [openode['settings']['minAnswerBodyLength']]
                                )
                }
...
DraftPost.prototype.decorate = function(element) {
    ...
    window.onbeforeunload = function() {
        ...
        //return interpolate(msg, [openode['data']['userName']]);
    };
};
...
DraftQuestion.prototype.getUrl = function() {
    return openode['urls']['saveDraftQuestion'];
};
...
DraftAnswer.prototype.getUrl = function() {
    return openode['urls']['saveDraftAnswer'];
};
...
CommentVoteButton.prototype.getVoteHandler = function(){
    ...
    return function(){
        ...
        $.ajax({
            ...
            data: data,
            dataType: 'json',
            url: openode['urls']['upvote_comment'],
            cache: false,
            success: function(data){
                if (data['success'] === true){
...
var Vote = function(){
    ...
    var pleaseLogin = " <a href='" + openode['urls']['user_signin']
                    + "?next=" + openode['urls']['question_url_template']
                    + "'>"
    ...
    var getAnswerVoteUpButtons = function(){
        ...
        var answerVoteUpButton = 'div.'+ voteContainerId +' a[id^="'+ imgIdPrefixAnswerVoteup +'"]';
        return $(answerVoteUpButton);
    };
    var getAnswerVoteDownButtons = function(){
        // var answerVoteDownButton = 'div.'+ voteContainerId +' div[id^="'+ imgIdPrefixAnswerVotedown +'"]';
        var answerVoteDownButton = 'div.'+ voteContainerId +' a[id^="'+ imgIdPrefixAnswerVotedown +'"]';
        return $(answerVoteDownButton);
    };
    var getAnswerVoteUpButton = function(id){
        // var answerVoteUpButton = 'div.'+ voteContainerId +' div[id="'+ imgIdPrefixAnswerVoteup + id + '"]';
        var answerVoteUpButton = 'div.'+ voteContainerId +' a[id="'+ imgIdPrefixAnswerVoteup + id + '"]';
        return $(answerVoteUpButton);
    };
    var getAnswerVoteDownButton = function(id){
        // var answerVoteDownButton = 'div.'+ voteContainerId +' div[id="'+ imgIdPrefixAnswerVotedown + id + '"]';
        var answerVoteDownButton = 'div.'+ voteContainerId +' a[id="'+ imgIdPrefixAnswerVotedown + id + '"]';
        return $(answerVoteDownButton);
    };
    ...
    var submit = function(object, voteType, callback) {
        ...
        $.ajax({
            ...
            cache: false,
            dataType: "json",
            url: openode['urls']['vote_url_template'].replace('{{QuestionID}}', questionId),
    ...
    var callback_accept = function(object, voteType, data){
        ...
        if(data.allowed == "0" && data.success == "0"){
            ...
            $(".answer-accept").removeClass("accepted");
...
var questionRetagger = function(){
    ...
    var doRetag = function(){
        $.ajax({
            type: "POST",
            url: retagUrl,//todo add this url to openode['urls']
            dataType: "json",
    ...
    var setupInputEventHandlers = function(input){
        ...
        // WTF: $(document).unbind('click').click(cancelRetag);
    ...
    var createRetagForm = function(old_tags_string){
        ...
        tagInput = $('<input id="retag_tags" type="text" autocomplete="off" name="tags" size="30"/> <a id="js-close-tags" class="button">close</a> <a id="js-save-tags" class="button success">save</a>');
        //var tagLabel = $('<label for="retag_tags" class="error"></label>');
        //populate input
        var tagAc = new AutoCompleter({
            url: openode['urls']['get_tag_list'],
            preloadData: true,
            minChars: 1,
            useCache: true,
            matchInside: true,
        ...
        form.validate({//copy-paste from utils.js
            rules: {
                tags: {
                    required: openode['settings']['tagsAreRequired'],
                    maxlength: openode['settings']['maxTagsPerPost'] * openode['settings']['maxTagLength'],
                    limit_tag_count: true,
                    limit_tag_length: true
                }
            },
    ...
    var getTagsAsString = function(tags_div){
        ...
        links.each(function(index, element){
            if (index === 0){
                ...
                tags_str = $(element).data("tag_name");
            } else {
                tags_str += ' ' + $(element).data("tag_name");
            }
        });
    ...
    var startRetag = function(){
        ...
        $("#js-close-tags").on("click", function(){
            cancelRetag();
        });
...
DeletePostLink.prototype.setPostDeleted = function(is_deleted){
    ...
    if (is_deleted === true && confirm(gettext('Are you shure to delete this post?'))){
...
EditCommentForm.prototype.getCounterUpdater = function(){
    ...
    var handler = function(){
        ...
        if (length === 0){
            feedback = interpolate(gettext('%s title minchars'), [chars]);
        }
        else if (length < 10){
            feedback = interpolate(gettext('enter %s more characters'), [chars - length]);
        }
        else {
...
EditCommentForm.prototype.createDom = function(){
    ...
    */
    ...
    if (openode['settings']['saveCommentOnEnter']){
...
EditCommentForm.prototype.getSaveHandler = function(){
    ...
    return function(){
        ...
        if (me._type == 'edit'){
            ...
            post_url = openode['urls']['editComment'];
        }
        else {
            post_data['post_type'] = me._comment.getParentType();
            post_data['post_id'] = me._comment.getParentId();
            post_url = openode['urls']['postComments'];
        }
...
var Comment = function(widget, data){
    ...
    this._element = null;
    this._is_convertible = openode['data']['userIsAdminOrMod'];
...
Comment.prototype.getDeleteHandler = function(){
    ...
    return function(){
        if (confirm(gettext('confirm delete comment'))){
            ...
            $.ajax({
                type: 'POST',
                url: openode['urls']['deleteComment'],
...
PostCommentsWidget.prototype.decorate = function(element){
    ...
    // this._is_truncated = openode['data'][widget_id]['truncated'];
    // this._user_can_post = openode['data'][widget_id]['can_post'];
...
PostCommentsWidget.prototype.showButton = function(){
    if (this._is_truncated === false){
        this._activate_button.html(openode['messages']['addComment']);
    }
    ...
    while (true){
        ...
        if (title.length >= 10){
            ...
            $.ajax({
                ...
                cache: false,
                dataType: "json",
                url: openode['urls']['swap_question_with_answer'],
                data: data,
                success: function(data){
                    var url_template = openode['urls']['question_url_template'];
 ...
 */
var OrganizationDescriptionEditor = function(){
    WrappedElement.call(this);
...
inherits(OrganizationDescriptionEditor, WrappedElement);

OrganizationDescriptionEditor.prototype.backupContent = function(){
    this._content_backup = this._content_box.contents();
};

OrganizationDescriptionEditor.prototype.setEnabledEditorButtons = function(buttons){
    this._enabled_editor_buttons = buttons;
};

OrganizationDescriptionEditor.prototype.setPreviewerEnabled = function(state){
    this._is_previewer_enabled = state;
...
OrganizationDescriptionEditor.prototype.setContent = function(content){
    this._content_box.empty();
    this._content_box.append(content);
};

OrganizationDescriptionEditor.prototype.setState = function(state){
    if (state === 'edit'){
        this._state = state;
        this._edit_btn.hide();
...
OrganizationDescriptionEditor.prototype.restoreContent = function(){
...
OrganizationDescriptionEditor.prototype.getTagId = function(){
    return this._tag_id;
};

OrganizationDescriptionEditor.prototype.isEditorLoaded = function(){
    return this._is_editor_loaded;
};

OrganizationDescriptionEditor.prototype.setEditorLoaded = function(){
 ...
 * the editor
 */
OrganizationDescriptionEditor.prototype.startActivatingEditor = function(){
    ...
    $.ajax({
        type: 'GET',
        url: openode['urls']['load_object_description'],
        data: data,
        cache: false,
        success: function(data){
            me.backupContent();
...
OrganizationDescriptionEditor.prototype.saveData = function(){
    var me = this;
    var text = this._editor.getText();
    var data = {
        object_id: me.getTagId(),
        model_name: 'Organization',//todo: fixme
        text: text
    };
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: openode['urls']['save_object_description'],
        data: data,
        cache: false,
        success: function(data){
            if (data['success']){
...
OrganizationDescriptionEditor.prototype.cancelEdit = function(){
    this.restoreContent();
    this.setState('display');
};

OrganizationDescriptionEditor.prototype.decorate = function(element){
    //expect <div id='organization-wiki-{{id}}'><div class="content"/><a class="edit"/></div>
    this._element = element;
...
var UserOrganizationProfileEditor = function(){
    OrganizationDescriptionEditor.call(this);
};
inherits(UserOrganizationProfileEditor, OrganizationDescriptionEditor);

UserOrganizationProfileEditor.prototype.toggleEmailModeration = function(){
    var btn = this._moderate_email_btn;
    var organization_id = this.getTagId();
    $.ajax({
        type: 'POST',
        dataType: 'json',
        cache: false,
        data: {organization_id: organization_id},
        url: openode['urls']['toggle_organization_email_moderation'],
        success: function(data){
            if (data['success']){
...
UserOrganizationProfileEditor.prototype.decorate = function(element){
    this.setEnabledEditorButtons('bold italic link ol ul');
    this.setPreviewerEnabled(false);
    UserOrganizationProfileEditor.superClass_.decorate.call(this, element);
    ...
    btn = element.find('#moderate-answers-to-enquirers');
    ...
    var selectorElement = element.find('#organization-openness-selector');
    ...
    logo_changer.setImageElement(element.find('.organization-logo'));
    logo_changer.setAjaxData({
        organization_id: this.getTagId()
    });
    logo_changer.setSaveUrl(openode['urls']['save_organization_logo_url']);
    logo_changer.setDeleteUrl(openode['urls']['delete_organization_logo_url']);
...
var OrganizationJoinButton = function(){
    TwoStateToggle.call(this);
};
inherits(OrganizationJoinButton, TwoStateToggle);

OrganizationJoinButton.prototype.getPostData = function(){
    return { organization_id: this._organization_id };
};

OrganizationJoinButton.prototype.getHandler = function(){
    var me = this;
    return function(){
        $.ajax({
            type: 'POST',
            ...
            cache: false,
            data: me.getPostData(),
            url: openode['urls']['join_or_leave_organization'],
            success: function(data){
                if (data['success']){
...
OrganizationJoinButton.prototype.decorate = function(elem) {
    OrganizationJoinButton.superClass_.decorate.call(this, elem);
    this._organization_id = this._element.data('organization-id');
};

var TagEditor = function() {
    WrappedElement.call(this);
    this._has_hot_backspace = false;
    this._settings = JSON.parse(openode['settings']['tag_editor']);
};
...
CategoryAdder.prototype.startAdding = function() {
    ...
    $.ajax({
        ...
        url: openode['urls']['add_tag_category'],
        cache: false,
        success: function(data) {
            if (data['success']) {
                //rebuild category tree based on data
...
CategorySelectorLoader.prototype.getOnLoadHandler = function() {
    ...
    return function(html){
        ...
        me.setEditor(editor);
        $('#js-tags-list').after(editor);

        var selector = openode['functions']['initCategoryTree']();
...
CategorySelectorLoader.prototype.startLoadingHTML = function(on_load) {
    ...
    $.ajax({
        ...
        url: openode['urls']['get_html_template'],
        cache: true,
        success: function(data) {
            if (data['success']) {
                on_load(data['html']);
...
CategorySelectorLoader.prototype.getSaveHandler = function() {
    ...
    return function() {
        ...
        $.ajax({
            type: "POST",
            url: retagUrl,//add to openode['urls']
            dataType: "json",
            data: { tags: getUniqueWords(tagInput.val()).join(' ') },
            success: function(json) {
                if (json['success'] === true){
...
CategorySelectorLoader.prototype.decorate = function(element) {
    this._element = element;
    this._display_tags_container = $('#js-tags-list');
...
$(document).ready(function() {
    ...
    publishBtns.each(function(idx, btn) {
        setupButtonEventHandlers($(btn), function() {
            ...
            $.ajax({
                ...
                url: openode['urls']['publishAnswer'],
                success: function(data) {
                    if (data['success']) {
    ...
    if (proxyUserNameInput.length === 1) {
        ...
        var fakeUserAc = new AutoCompleter({
            url: '/get-users-info/',//openode['urls']['get_users_info'],
            preloadData: true,
    ...
    var showSharedOrganizations = $('.see-related-organizations');
    if (showSharedOrganizations.length) {
        var OrganizationsPopup = new ThreadUsersDialog();
        OrganizationsPopup.setHeadingText(gettext('Shared with the following organizations:'));
        OrganizationsPopup.decorate(showSharedOrganizations);
    }
});
