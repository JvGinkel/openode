...
TagDetailBox.prototype.loadTags = function(wildcard, callback){
    ...
    $.ajax({
        ...
        dataType: 'json',
        cache: false,
        url: openode['urls']['get_tags_by_wildcard'],
...
function pickedTags(){
    ...
    var sendAjax = function(tagnames, reason, action, callback){
        ...
        if (action == 'add') {
            if (reason == 'good') {
                url = openode['urls']['mark_interesting_tag'];
            } else  if (reason == 'bad') {
                url = openode['urls']['mark_ignored_tag'];
            } else {
                url = openode['urls']['mark_subscribed_tag'];
            }
        }
        else {
            url = openode['urls']['unmark_tag'];
        }
    ...
    var setupTagFilterControl = function(control_type){
        ...
        .click(function(){
            $.ajax({
                ...
                dataType: 'json',
                cache: false,
                url: openode['urls']['set_tag_filter_strategy'],
                data: {
