=====================
Skin system in Pluto
=====================

This document aims to help web designers customize skin for their pluto instances.

Pluto has own skinning system, where current skin can be switched on the fly
...
Skin is a directory, pointed to by ``PLUTO_EXTRA_SKINS_DIR``
...
python code of pluto. In addition, there are templates that are
included
...
would you like to customize pluto's appearance.
...
---------------------------------------------
In this method you will not need to edit any pluto's files.
The ``extra.css`` file is not distributed with pluto, but can be
added by the site administrators wishing to add their own
css rules to those shipped with pluto.
...
is via use git revision control on a clone of the pluto
master repository. It does require some knowledge of git system.

If you plan to do this, firstly, install pluto from the repository.
In addition, it will help if your copy of pluto code is installed
in the django project directory (use ``python setup.py develop`` method
to install pluto in the first place).

Then edit anything in directories ``pluto/templates`` and ``pluto/media``
and commit to your own repository.

If the pluto app is installed in the `site-packages` or `dist-packages`
...
See also: :ref:`customizing-style-css-file-in-pluto`.
...
hard-coded in the pluto's python code.

Add setting ``PLUTO_EXTRA_SKINS_DIR`` to your ``settings.py`` file
and set its value to the directory with your additional skins.

For example::

    PLUTO_EXTRA_SKINS_DIR = '/home/myname/my_pluto_themes'

And your directory structure might be::

    /home/myname/my_pluto_themes/
                          /my_theme
                                /templates
                                /media
...
The first template to look at is `pluto/templates/base.html`, it is quite simple and you can substantially change the appearance by modifying that template in the combination with adding some custom css.
...
Some pages in pluto have classes assigned to the HTML ``<body>`` element,
...
|                                    | functions needed for pluto this is included at the  |
...
+----------------------------+------------------------------+--------------------------------+
| contributors.html          | club/sidebar.html       | Widget to display contributors | 
|                            |                              | avatars.                       |
+----------------------------+------------------------------+--------------------------------+
...
+----------------------------+------------------------------+--------------------------------+
| related_tags.html          | club/sidebar.html       | Question operation links, edit | 
|                            |                              | report as spam and more.       |
+----------------------------+------------------------------+--------------------------------+
...
|                            |                              | sharing buttons.               |
+----------------------------+------------------------------+--------------------------------+
| tag_selector.html          | club/sidebar.html       | Contains the tag selector for  | 
|                            |                              | search.                        |
+----------------------------+------------------------------+--------------------------------+
