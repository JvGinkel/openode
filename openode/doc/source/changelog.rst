...
* Added setting `PLUTO_INTERNAL_IPS` - to allow anonymous access to 
  closed sites from dedicated IP addresses (Evgeny)
* Moved default skin from `pluto/skins/default` to simply `pluto` (Evgeny)
...
* Made email recovery link work when pluto is deployed on subdirectory (Evgeny)
* Added tests for the CSRF_COOKIE_DOMAIN setting in the startup_procedures (Evgeny)
* Pluto now respects django's staticfiles app (Radim Řehůřek, Evgeny)
...
* replaced setting `PLUTO_FILE_UPLOAD_DIR` with django's `MEDIA_ROOT` (Evgeny)
* replaced setting `PLUTO_UPLOADED_FILES_URL` with django's `MEDIA_URL` (Evgeny)
...
0.7.31 (Nov 29, 2011)
---------------------
* Added ``pluto_create_test_fixture`` management command (Dejan Noveski)
...
* Added management command ``pluto_add_test_content`` (Dejan Noveski)
...
* Resolved `bug #102<http://bugs.pluto.org/issues/102>`_ - duplicate notifications on posts with mentions (Evegeny)
* Added color-animated transitions when urls with hash tags are visited (Adolfo)
* Repository tags will be `automatically added <http://pluto.org/en/question/345/can-git-tags-be-created-for-each-of-the-releases>`_ to new releases (Evgeny, suggsted by ajmirsky)
...
* Django's createsuperuser now works with pluto (Adolfo Fitoria)
...
* `Fixed "Moderation tab is misaligned" issue reported by methner. <http://pluto.org/en/question/587/moderation-tab-is-misaligned-fixed>`_
* Fixed bug in follow users and changed the follow button design.

0.7.18
------
* `Added multiple capitalization to username mentions(reported by niles) <http://pluto.org/en/question/580/allow-alternate-capitalizations-in-user-links>`_
...
* New inbox system style (`bug reported by Tomasz P. Szynalski <http://pluto.org/en/question/470/answerscomments-are-listed-twice-in-the-inbox>`_).
