...
Pluto supports a number of optional modules, enabling certain features, not available 
in pluto by default.
...
Sphinx search
=============
Pluto supports Sphinx search - and at this point only for MySQL.
...
* prepare configuration file by running command ``python manage.py get_pluto_sphinx_config > sphinx.conf``
...
In ``settings.py`` add::
    ...
    PLUTO_SPHINX_SEARCH_INDEX = 'pluto'
...
Initialize the sphinx index (may need to log in as root)::
    ...
    indexer pluto --config /etc/sphinx/sphinx.conf
...
Set up a periodic re-indexing job (using cron)::
    ...
    indexer pluto --rotate --config /etc/sphinx/sphinx.conf
...
Haystack search
=============
Pluto supports `Haystack <http://haystacksearch.org/>`_, a modular search framework that supports popular search engine backends as 
Solr, Elasticsearch, Whoosh and Xapian. 

.. note::
    Haystack support in Pluto is a new feature,
    please give us your feedback at ``support@pluto.com``
...
Want to share videos in pluto posts? It is possible, but you will have to install a forked 
...
"Settings for pluto data entry and display" and check "Enable embedding video".
...
* ``LDAP_AUTHENTICATE_FUNCTION`` - dotted python path to optional function that
  ...
  `pluto.deps.django_authopenid.ldap_auth.ldap_authenticate_default`.
...
* ``LDAP_CREATE_USER_FUNCTION`` - python dotted path to function that will create
  ...
  ``pluto.deps.django_authopenid.ldap_auth.ldap_create_user_default``.
...
The function to look at is `pluto.deps.django_authopenid.backends.ldap_authenticate`.
If you have problems with LDAP please contact us at support@pluto.com.

The easiest way to debug - insert ``import pdb; pdb.set_trace()`` line into function
`pluto.deps.django_authopenid.backends.ldap_authenticate`,
...
Add a following setting to your ``settings.py``::
    ...
    PLUTO_CUSTOM_USER_PROFILE_TAB = {
        'NAME': 'some name',
        'SLUG': 'some-name',
        'CONTENT_GENERATOR': 'myapp.views.somefunc'
    }

The value of ``PLUTO_CUSTOM_USER_PROFILE_TAB['CONTENT_GENERATOR']``
...
* Go back to your pluto site settings and click on *Login Provider Settings* and then activate the option *Activate to allow login with self-hosted wordpress site*, 
...
Pluto supports `celery <http://celeryproject.org/>`_ distributed task queue for some task, to enable it follow the following steps:
...
For `supervisor <http://supervisord.org/>`_: add this sample config file named pluto.conf into /etc/supervisor/conf.d/ directory::

    [program:pluto_celery]
    ...
    user=nobody
    numprocs=1
    stdout_logfile=/var/log/pluto_celery.log
    stderr_logfile=/var/log/pluto_celery.err
    autostart=true
    autorestart=true
    startsecs=10
...
Pluto supports posting replies by email. For this feature  to work ``Lamson`` and ``django-lamson`` need to be installed on the system. To install all the necessery dependencies execute the following command:
    
    pip install django-lamson

.. note::
    On Windows installation of the Lamson module may require
    additional work. Pluto does not support this feature
...
The minimum settings required to enable this feature are defining the port and binding address for the lamson SMTP daemon and the email handlers within pluto. Edit your settings.py file to include the following::

    LAMSON_RECEIVER_CONFIG = {'host': 'your.ip.address', 'port': 25}
    LAMSON_HANDLERS = ['pluto.mail.lamson_handlers']
...
Within the pluto admin interface there are 4 significant configuration points for this feature.
