...
+---------------------------------+-------------------------------------------------------------+
| `add_pluto_user --user-name    | Create a user account. If password is not given, an         |
...
|                                 | command                                                     |
+---------------------------------+-------------------------------------------------------------+
| `load_stackexchange <file.zip>` | Load SackExchange dump into Pluto. It is best to run this  |
|                                 | command on empty database. Also - before running, make sure |
|                                 | that `pluto.importers.stackexchange` is in the list of     |
...
These commands deal with the periodic tasks related to sending and receiving email by pluto.
...
|                                     | pluto will validate the email and capture the signature in |
...
|                                     | on :doc:`sending email to pluto <sending-email-to-pluto>`.|
...
.. note::
 ...
 Data inconsistency in the Pluto project is considered as a critical error and as a matter of 
 the project policy is addressed on the day of reporting. If you discover such issue - please
 report it at the forum or by email at `admin@pluto.org`
...
the developers of the Pluto project:
...
|                                | by the Pluto project. **Note:** the `jinja2_makemessages`  |
|                                | must be run from the `pluto` app directory, unlike all the |
|                                | remaining commands that are expected to be run from the     |
|                                | site root directory.                                        |
+--------------------------------+-------------------------------------------------------------+
| `pluto_add_test_content`      | Creates content with dummy data for testing                 |
+--------------------------------+-------------------------------------------------------------+
| `pluto_create_test_fixture`   | Creates a test fixture at `pluto/tests/test_data.json`     |
+--------------------------------+-------------------------------------------------------------+
