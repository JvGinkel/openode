...
from openode.models import ReplyAddress, Organization, Tag
...
def ASK(message, host = None, addr = None):
    ...
    if addr == 'ask':
        ...
        if openode_settings.GROUP_EMAIL_ADDRESSES_ENABLED == False:
            return
        try:
            group = Organization.objects.get(name__iexact=addr)
            mail.process_emailed_question(
                from_address, subject, body_text, stored_files,
                group_id = group.id
            )
