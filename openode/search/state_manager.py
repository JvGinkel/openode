...
class SearchState(object):
    ...
    def __init__(self, node, module, scope, sort, tags, author, page, user_logged_in, request = None):
        ...
        if sort is not None:
            sort_method, sort_dir = sort.split('-')
            if sort_method not in const.THREAD_SORT_METHOD_LIST or sort_dir not in const.THREAD_SORT_DIR_LIST:
        ...
        if node:
            self._questions_url = urlresolvers.reverse('node_module', kwargs={'node_id': node.pk, 'node_slug': node.slug, 'module': module})
    ...
    def change_sort(self, sort_method, sort_dir):
        ss = self.deepcopy()
