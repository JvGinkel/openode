...
title will be moved to the pluto_thread table and this function
will be simpler.
*/
...
DECLARE
    ...
    onerow record;
BEGIN
    query = 'SELECT text FROM pluto_post WHERE thread_id=' || thread_id ||
...
DECLARE
    ...
    onerow record;
BEGIN
    query = 'SELECT concat_tsvectors(text_search_vector) FROM pluto_post' ||
...
BEGIN
    query = 'SELECT concat_tsvectors(text_search_vector) ' ||
       'FROM pluto_post WHERE thread_id = ' || thread_id ||
       ' AND deleted=false';
    FOR onerow IN EXECUTE query LOOP
        RETURN onerow.concat_tsvectors;
    END LOOP;
...
SELECT add_tsvector_column('text_search_vector', 'pluto_thread');
SELECT add_tsvector_column('text_search_vector', 'pluto_post');

/* populate tsvectors with data */
-- post tsvectors
UPDATE pluto_post set text_search_vector = get_post_tsv(text, 'comment') WHERE post_type='comment';
UPDATE pluto_post SET text_search_vector = get_post_tsv(text, 'answer') WHERE post_type='answer';
UPDATE pluto_post SET text_search_vector = get_post_tsv(text, 'question') WHERE post_type='question';
UPDATE pluto_post as q SET text_search_vector = text_search_vector ||
    get_dependent_comments_tsv(q.id) WHERE post_type IN ('question', 'answer');

--thread tsvector
UPDATE pluto_thread SET text_search_vector = get_thread_tsv(title, tagnames);
UPDATE pluto_thread as t SET text_search_vector = text_search_vector ||
...
END;
$$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS thread_search_vector_update_trigger on pluto_thread;
CREATE TRIGGER thread_search_vector_update_trigger 
BEFORE UPDATE ON pluto_thread FOR EACH ROW EXECUTE PROCEDURE thread_update_trigger();
...
END;
$$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS thread_search_vector_insert_trigger on pluto_thread;
CREATE TRIGGER thread_search_vector_insert_trigger
BEFORE INSERT ON pluto_thread FOR EACH ROW EXECUTE PROCEDURE thread_insert_trigger();
...
BEGIN
    ...
    END IF;
    UPDATE pluto_thread SET id=new.thread_id WHERE id=new.thread_id;
    return new;
END;
$$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS post_search_vector_update_trigger on pluto_post;
CREATE TRIGGER post_search_vector_update_trigger 
BEFORE INSERT OR UPDATE ON pluto_post FOR EACH ROW EXECUTE PROCEDURE post_trigger();

DROP INDEX IF EXISTS pluto_search_idx;
CREATE INDEX pluto_search_idx ON pluto_thread USING gin(text_search_vector);
