...
from openode.user_messages import create_message, get_and_delete_messages
    ...
    def process_request(self, request):
        ...
        if request.user.is_anonymous():
            ...
            if 'greeting_set' not in request.session and \
                    'openode_visitor' not in request.COOKIES and \
                    openode_settings.ENABLE_GREETING_FOR_ANON_USER:
                request.session['greeting_set'] = True
                msg = _('First time here? Start searching in the field above or explore content.')
                request.user.message_set.create(message=msg)

    def process_response(self, request, response):
        """Adds the ``'openode_visitor'``key to cookie if user ever
        ...
        if hasattr(request, 'user') and \
                ...
                'openode_visitor' not in request.COOKIES :
            #import datetime
            ...
            #                        "%a, %d-%b-%Y %H:%M:%S GMT")
            response.set_cookie('openode_visitor', False)
        return response
